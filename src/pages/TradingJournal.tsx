...
const StatsCard: React.FC<{
    ...
    valueColor?: string;
}> = ({ title, value, subtext, icon, valueColor }) => (
            ...
            {isLoadingAlerts ? (
                ...
                <div className="space-y-2">
                    ...
                    {alerts.map((alert) => (
                        <div
                            key={alert.id}
                            className="bg-card-bg border border-gray-700/50 rounded-lg p-4 flex items-center justify-between hover:bg-gray-800/50 transition-colors"
                        >
                            <div className="flex items-center gap-4">
                                <div
                                    className={`p-2 rounded-full ${alert.status === AlertStatus.LIVE ? 'bg-yellow-500/10 text-yellow-500' : 'bg-blue-500/10 text-blue-500'}`}
                                >
                            ...
                            <span
                                className={`px-2 py-1 rounded text-xs font-bold uppercase ${alert.status === AlertStatus.LIVE ? 'bg-yellow-500/20 text-yellow-400' : 'bg-gray-700 text-gray-400'}`}
                            >
                                {alert.status}
                            </span>
                        </div>
                            ...
                            <div className="flex gap-2">
                                <div className="flex bg-gray-800 rounded-lg border border-gray-700 flex-1 sm:flex-none">
                                    <button
                                        ...
                                        className={`flex-1 sm:flex-none p-2 rounded-l-lg ${viewMode === 'list' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 sm:flex-none p-2 rounded-r-lg ${viewMode === 'gallery' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}
                                    >
                            ...
                            <div className="lg:col-span-1 space-y-3 md:space-y-4">
                                <div className="grid grid-cols-2 gap-3 md:gap-4">
                                    ...
                                    <StatsCard
                                        ...
                                        icon={<MarketIcon className="w-4 h-4 md:w-5 md:h-5" />}
                                    />
                                    <StatsCard
                                        title="Streak"
                                    ...
                                    <span
                                        className={
                                            ...
                                            chartData[chartData.length - 1].pnl >= 0
                                                ...
                                                : 'text-red-400 font-bold text-sm md:text-base'
                                        }
                                    >
                                        {chartData.length > 0
                        ...
                        {filteredEntries.length === 0 ? (
                            ...
                            >
                                {filteredEntries.map((entry) => (
                                    <div
                                        key={entry.id}
                                        className="bg-card-bg border border-gray-700/50 rounded-xl p-5 hover:border-blue-500/30 hover:bg-gray-800/30 transition-all group shadow-sm flex flex-col h-full relative overflow-hidden"
                                    >
                                        {/* Status Line */}
                                        <div
                                            className={`absolute left-0 top-0 bottom-0 w-1 ${entry.pnl && entry.pnl > 0 ? 'bg-green-500/50' : entry.pnl && entry.pnl < 0 ? 'bg-red-500/50' : 'bg-gray-700'}`}
                                        ></div>
                                        ...
                                        <div className="flex justify-between items-start mb-3 pl-2">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                                                    ...
                                                    {entry.symbol && (
                                                        <span className="px-1.5 py-0.5 bg-gray-800 rounded text-blue-300 font-mono border border-gray-700">
                                                            {entry.symbol}
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-3 pl-2">
                                            <span
                                                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${SENTIMENT_COLORS[entry.sentiment] || SENTIMENT_COLORS['Neutral']}`}
                                            >
                                                {entry.sentiment}
                                            </span>
                                            {entry.pnl !== undefined && (
                                                <span
                                                    className={`px-2 py-0.5 rounded text-[10px] font-bold tracking-wide border ${entry.pnl >= 0 ? 'text-green-400 bg-green-400/10 border-green-500/20' : 'text-red-400 bg-red-400/10 border-red-500/20'}`}
                                                >
                                                    {entry.pnl > 0 ? '+' : ''}
                                                    {entry.pnl} USD
                                                </span>
                                            ...
                                            <div className="flex gap-2 mb-3 pl-2 overflow-x-auto pb-1 scrollbar-hide">
                                                {entry.images.map((img, i) => (
                                                    <img
                                                        ...
                                                        src={img}
                                                        alt="chart"
                                                        className="h-16 w-24 object-cover rounded-lg border border-gray-700 hover:opacity-90 transition-opacity"
                                                    />
                                                ))}
                                            </div>
                                        )}
            ...
            <div className="bg-[#18181b] w-full sm:max-w-3xl sm:rounded-xl rounded-t-2xl sm:rounded-t-xl border-t sm:border border-gray-700 shadow-2xl overflow-hidden flex flex-col h-[95vh] sm:max-h-[90vh]">
                <div className="p-3 md:p-4 border-b border-gray-700 flex justify-between items-center bg-[#202024]">
                    <h3 className="font-bold text-white text-base md:text-lg flex items-center gap-2">
                        ...
                        <span className="text-[10px] md:text-xs font-normal text-gray-500 bg-gray-800 px-2 py-0.5 rounded hidden md:inline-block">
                            Paste (Ctrl+V) images directly
                        </span>
                    </h3>
                    <button
                        onClick={onClose}
                ...
                <div
                    className="p-4 md:p-6 overflow-y-auto space-y-3 md:space-y-4 custom-scrollbar"
                    onPaste={handlePaste}
                >
                    <div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                            {images.map((img, i) => (
                                <div
                                    key={i}
                                    className="relative group aspect-video bg-gray-900 rounded-lg border border-gray-700 overflow-hidden"
                                >
                                    <img
                                        src={img}
                                        alt="attachment"
                ...
                <div className="p-3 md:p-4 border-t border-gray-700 bg-[#202024] flex justify-end gap-2 md:gap-3 flex-shrink-0">
                    <button
                        onClick={onClose}
                        className="px-4 py-2.5 md:py-2 text-gray-400 hover:text-white text-sm font-medium"
                    >
                        Cancel
                    </button>
                    <button
