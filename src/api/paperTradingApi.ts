...
export type PaperTradingAccountInput = Omit<
    PaperTradingAccount,
...
export async function deletePaperTradingAccount(id: string): Promise<void> {
    const { error } = await db().from('paper_trading_accounts').delete().eq('id', id);
...
export async function transferFunds(
    ...
    const { data: fromAccount, error: fetchFromError } = await db()
        .from('paper_trading_accounts')
        .select('balance')
        .eq('id', fromAccountId)
        .single();
    ...
    const { data: toAccount, error: fetchToError } = await db()
        .from('paper_trading_accounts')
        .select('balance')
        .eq('id', toAccountId)
        .single();
