...
const EditWatchlistNameModal: React.FC<EditWatchlistNameModalProps> = ({
    ...
    const [stopLossDistance, setStopLossDistance] = useState(
        currentRiskSettings?.stopLossDistance || 0
    );
    const [takeProfitDistance, setTakeProfitDistance] = useState(
        currentRiskSettings?.takeProfitDistance || 0
    );
    const [trailingStopLossDistance, setTrailingStopLossDistance] = useState(
        currentRiskSettings?.trailingStopLossDistance || 0
    );
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-8">
                        ...
                        <div className="space-y-4">
                            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider border-b border-gray-700 pb-2">
                                General Settings
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-gray-500 mt-1">
                                        Restrict execution to signals from this specific timeframe.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <CustomSelect
                                    ...
                                    onSelect={(value) =>
                                        setTradingMode(value === 'Paper Trading' ? 'paper' : 'live')
                                    }
                                />
                            </div>
                            ...
                            {isCrypto && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <CustomSelect
                                        ...
                                        onSelect={(val) =>
                                            setMarketType(val === 'Spot' ? 'spot' : 'futures')
                                        }
                                    />
                                    <CustomSelect
                                        label="Risk Method"
                                        ...
                                        onSelect={(val) =>
                                            setRiskMethod(
                                                val === 'Fixed Amount ($)' ? 'fixed' : 'percent'
                                            )
                                        }
                                    />
                                </div>
                                        ...
                                        {marketType === 'futures' && (
                                            <div>
                                                ...
                                                {autoLeverageEnabled &&
                                                    riskMethod === 'percent' && (
                                                        <p className="text-[10px] text-gray-500 mt-1">
                                                            Leverage calculated dynamically
                                                        </p>
                                                    )}
                                            </div>
                                        )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setStopLossDistance(Number(e.target.value))
                                        }
                                        className="w-full bg-gray-900/50 border border-gray-700 rounded p-2 text-sm text-white"
                                    />
                                    <p className="text-[10px] text-gray-500 mt-1">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setTakeProfitDistance(Number(e.target.value))
                                        }
                                        className="w-full bg-gray-900/50 border border-gray-700 rounded p-2 text-sm text-white"
                                    />
                                    <p className="text-[10px] text-gray-500 mt-1">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setTrailingStopLossDistance(Number(e.target.value))
                                        }
                                        className="w-full bg-gray-900/50 border border-gray-700 rounded p-2 text-sm text-white"
                                    />
                                    <p className="text-[10px] text-gray-500 mt-1">
                                        Distance to trail price
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex justify-end items-center p-4 bg-gray-900/50 border-t border-gray-700 rounded-b-lg gap-3 sticky bottom-0">
                        <button
                            type="button"
                            onClick={onClose}
                            className="px-5 py-2 rounded-md text-sm font-semibold text-gray-300 hover:bg-gray-700/50"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={!name.trim()}
                            className="px-6 py-2 rounded-md text-sm font-semibold bg-blue-500 text-white hover:bg-blue-600 disabled:bg-gray-600 disabled:cursor-not-allowed"
                        >
                            Save Changes
                        </button>
                    </div>
