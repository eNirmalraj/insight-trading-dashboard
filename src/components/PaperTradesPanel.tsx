...
export const PaperTradesPanel: React.FC = () => {
    ...
    if (loading && trades.length === 0)
        return <div className="p-4 text-emerald-400">Loading Paper Trades...</div>;
                    ...
                    <tbody className="divide-y divide-white/5">
                        {trades.map((trade) => (
                            <tr key={trade.id} className="hover:bg-white/5 transition-colors">
                                ...
                                <td
                                    className={`px-4 py-3 font-medium ${trade.direction === 'BUY' ? 'text-emerald-400' : 'text-rose-400'}`}
                                >
                                    <div className="flex flex-col">
                                        <span>{trade.direction}</span>
                                        {trade.direction === 'SELL' &&
                                            !trade.symbol.endsWith('.P') && (
