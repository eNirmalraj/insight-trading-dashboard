        ...
        <div className="relative" ref={wrapperRef}>
            <div className="flex relative group">
                <button
                    ...
                    className={`p-2 rounded-md flex items-center justify-center transition-colors relative ${isActive ? 'bg-[#2C2C2C] text-[#2962FF]' : 'text-[#E0E0E0] hover:bg-[#1E1E1E] hover:text-white'}`}
                    title={displayTool.name}
                >
                    {displayTool.icon}
                    {/* Small triangle indicator for group */}
                    <div
                        className={`absolute bottom-[2px] right-[2px] w-[6px] h-[6px] pointer-events-none transition-colors ${isActive ? 'text-[#2962FF]' : 'text-[#9E9E9E] group-hover:text-[#FFFFFF]'}`}
                    >
            ...
            {isOpen && (
                <div className="absolute right-full top-0 mr-2 bg-[#1E1E1E] border border-[#2A2A2A] rounded-md shadow-xl z-50 flex flex-col min-w-[180px] py-1 animate-in fade-in slide-in-from-right-2 duration-100">
                    <div className="px-3 py-2 text-[10px] font-bold text-[#9E9E9E] uppercase border-b border-[#2A2A2A] mb-1 tracking-wider">
                        {category}
                    </div>
                    {tools.map((tool) => (
                        <button
                            ...
                            className={`flex items-center gap-3 px-3 py-2 text-[14px] text-left transition-colors hover:bg-[#2C2C2C] ${tool.name === displayTool.name ? 'bg-[#2962FF]/10 text-[#2962FF]' : 'text-[#E0E0E0]'}`}
                        >
...
const DrawingToolbar: React.FC<DrawingToolbarProps> = ({ tools, activeTool, onToolSelect }) => {
    ...
    return (
        <div className="flex flex-col items-center gap-1.5 py-4">
            <div className="w-8 h-px bg-[#1E1E1E] mb-2"></div>
            {order.map((cat) => {
