...
const HeaderButton: React.FC<{
    ...
    onClick?: () => void;
    disabled?: boolean;
}> = ({ children, className, title, onClick, disabled }) => (
    <button
        title={title}
        onClick={onClick}
        disabled={disabled}
        className={`flex items-center justify-center p-1.5 rounded-md text-gray-400 hover:bg-gray-700 hover:text-white transition-colors disabled:text-gray-600 disabled:cursor-not-allowed disabled:hover:bg-transparent ${className}`}
    >
        {children}
    </button>
);
                ...
                <div className="bg-gray-800 p-0.5 rounded-lg flex items-center gap-1">
                    <div className="relative">
                        <button
                            ...
                            className="px-2 py-0.5 rounded-md hover:bg-gray-700"
                        >
                                    ...
                                    ([groupName, tfs]) =>
                                        tfs.length > 0 && (
                                            <div key={groupName} className="mt-2">
                                                <h4 className="text-xs text-gray-500 font-semibold px-2 mb-1">
                                                    {groupName}
                                                </h4>
                                                <div className="space-y-1">
                                                    {tfs.map((tf) => (
                                                        <div
                                                            key={tf}
                                                            className={`flex items-center justify-between p-1 rounded-md transition-colors ${activeTimeframe === tf ? 'bg-blue-500/20' : 'hover:bg-gray-700/50'}`}
                                                        >
                                                            <button
                                                                onClick={() => {
                                                            ...
                                                            >
                                                                <StarIcon
                                                                    className={`w-4 h-4 transition-colors ${favoriteTimeframes.includes(tf) ? 'text-yellow-400' : 'text-gray-600 hover:text-yellow-500'}`}
                                                                />
                                                            </button>
                                                        </div>
                                                    ))}
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="bg-gray-800 p-1 rounded-lg">
                    <div className="hidden md:flex items-center gap-1">
                        ...
                        <HeaderButton onClick={onToggleFullscreen} title="Fullscreen">
                            <FullscreenIcon className="w-5 h-5" />
                        </HeaderButton>
                        <HeaderButton onClick={onTakeSnapshot} title="Take a snapshot">
