// cspell:ignore watchlist forex ohlc
...
const WatchlistPanel: React.FC<WatchlistPanelProps> = ({ onClose, onSymbolSelect, symbol }) => {
    ...
    const [tickerData, setTickerData] = useState<
        Record<string, { price: number; changePercent: number; volume: number; change: number }>
    >({});
    ...
    const handleCopyToScripts = async () => {
        ...
        try {
            ...
            const newScript = await api.createScript(
                newScriptName,
                activeWatchlist.accountType,
                activeWatchlist.strategyType || 'Manual'
            );

            // Add items concurrently
            await Promise.all(
                activeWatchlist.items.map((item) =>
                    api.addSymbolToScript(newScript.id, item.symbol)
                )
            );
    ...
    return (
        <>
            {resizingColumn && (
                <div className="fixed top-0 left-0 w-full h-full z-[9999] cursor-col-resize select-none" />
            )}
            <div
            ...
            >
                        ...
                        <div ref={settingsRef} className="relative">
                            ...
                            {isSettingsOpen && (
                                <div className="absolute top-full right-0 mt-2 bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-2 z-50 w-40">
                                    <div className="text-xs font-bold text-gray-500 uppercase px-2 py-1 mb-1">
                                        Columns
                                    </div>
                ...
                <div className="p-1.5 border-b border-gray-700/50 flex items-center gap-2">
                    <div ref={wlDropdownRef} className="relative flex-1">
                        <button
                            onClick={() => setIsWlDropdownOpen((p) => !p)}
                            className="w-full flex justify-between items-center bg-gray-800 p-2 rounded-md text-sm hover:bg-gray-700 transition-colors"
                        >
                            <span>{activeWatchlist?.name || 'Select a list'}</span>
                            <ChevronDownIcon
                                className={`w-4 h-4 transition-transform ${isWlDropdownOpen ? 'rotate-180' : ''}`}
                            />
                        </button>
                        {isWlDropdownOpen && (
                ...
                <div ref={containerRef} className="flex-1 overflow-hidden">
                    <style>{`
                        ${visibleColumns.map((col) => `.width-col-${col.id} { width: ${col.id === lastColumnId ? 'auto' : `${columnWidths[col.id]}%`}; }`).join('\n')}
                    `}</style>
                    <table className="w-full text-xs md:text-sm table-fixed">
                            ...
                            <tr>
                                {visibleColumns.map((col) => {
                                    ...
                                    return (
                                        <th
                                            key={col.id}
                                            className={`p-2 text-center relative border-r border-gray-800/50 last:border-r-0 width-col-${col.id}`}
                                        >
                                            {col.label}
                                            {!isLast && (
                                                <div
                        ...
                        <tbody>
                            {filteredItems.map((item) => {
                                ...
                                const displayChangePercent = hasData
                                    ? data.changePercent
                                ...
                                return (
                                    <tr
                                        key={item.id}
                                        className="group hover:bg-gray-800 border-b border-gray-800/50 relative"
                                    >
                                        {visibleColumns.map((col) => {
                                            const isLast = col.id === lastColumnId;
                                            ...
                                            if (col.id === 'symbol') {
                                                return (
                                                    <td
                                                        ...
                                                        className={`p-1.5 font-medium cursor-pointer border-r border-gray-800/50 last:border-r-0 text-center width-col-${col.id}`}
                                                    >
                                            ...
                                            if (col.id === 'price') {
                                                ...
                                                content =
                                                    displayPrice > 0
                                                        ? `${isPos ? '+' : ''}${displayChange.toFixed(displayPrice < 1 ? 5 : 2)}`
                                                        : '-';
                                                cellColor = colorClass;
                                            } else if (col.id === 'changePercent') {
                                                content =
                                                    displayPrice > 0
                                                        ? `${isPos ? '+' : ''}${displayChangePercent.toFixed(2)}%`
                                                        : '-';
                                                cellColor = colorClass;
                                            ...
                                            return (
                                                <td
                                                    key={col.id}
                                                    onClick={() => onSymbolSelect(item.symbol)}
                                                    className={`p-1.5 text-center cursor-pointer ${cellColor} border-r border-gray-800/50 last:border-r-0 width-col-${col.id}`}
                                                >
                                                    {content}
                                                </td>
                                            );
            ...
            {isAddSymbolModalOpen && activeWatchlist && (
                <SymbolSearchModal
                    ...
                    defaultTab={
                        activeWatchlist.accountType === AccountType.CRYPTO
                            ? 'Crypto'
                            : activeWatchlist.accountType === AccountType.FOREX
                              ? 'Forex'
                              : activeWatchlist.accountType === AccountType.INDIAN
                                ? 'All'
                                : 'All'
                    }
                    allowedTabs={
                                ...
                                <div className="flex justify-between items-center">
                                    <div className="flex-1 pr-2">
                                        <div className="flex items-center justify-between mb-1">
                                            <span
                                                className={`px-2 py-0.5 text-[10px] font-semibold rounded-full ${alert.triggered ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}
                                            >
...
const ObjectTreePanel: React.FC<{
    ...
    const getDrawingName = (d: Drawing): string => {
        switch (d.type) {
            case 'Horizontal Line':
                return `${d.type} (${d.price.toFixed(5)})`;
            case 'Text Note':
                return `${d.type} ("${d.text.substring(0, 15)}...")`;
            default:
                return d.type;
        }
    };
    ...
    return (
        <div className="w-full h-full flex flex-col text-white bg-gray-900">
            <style>{`
                ${indicators.map((i) => `.indicator-color-${i.id.replace(/[^a-zA-Z0-9]/g, '')} { color: ${i.isVisible ? i.settings.color : '#6B7280'}; }`).join('\n')}
            `}</style>
            <div className="flex items-center justify-between p-2.5 border-b border-gray-700/50">
                ...
                {drawings.length === 0 && indicators.length === 0 ? (
                    <div className="text-center py-16 px-6 text-gray-500">
                        ...
                        <p className="mt-2 text-xs">
                            Use the toolbar to add drawings or indicators.
                        </p>
                    </div>
                ) : (
                    <>
                        {drawings.length > 0 && (
                            <section className="mb-4">
                                <h3 className="px-2 text-xs font-semibold text-gray-400 mb-2">
                                ...
                                <ul className="space-y-1">
                                    {drawings.map((d) => (
                                        <li
                                            key={d.id}
                                            className="flex items-center justify-between p-2 rounded-md hover:bg-gray-800 group"
                                        >
                                            ...
                                            <div className="flex items-center gap-3 opacity-100 md:opacity-0 md:group-hover:opacity-100">
                                                <button
                                                    onClick={() => onToggleDrawingVisibility(d.id)}
                                                    title={d.isVisible === false ? 'Show' : 'Hide'}
                                                >
                                                    {d.isVisible === false ? (
                                                        <EyeOffIcon className="w-4 h-4 text-gray-400 hover:text-white" />
                                                    ) : (
                            ...
                            <section>
                                ...
                                <ul className="space-y-1">
                                    {indicators.map((i) => (
                                        ...
                                        >
                                            <span
                                                className={`text-sm indicator-color-${i.id.replace(/[^a-zA-Z0-9]/g, '')}`}
                                            >
                                                {i.type}
                                            </span>
                                            <div className="flex items-center gap-3 opacity-100 md:opacity-0 md:group-hover:opacity-100">
                                                <button
                                                    onClick={() =>
                                                        onToggleIndicatorVisibility(i.id)
                                                    }
                                                    title={i.isVisible ? 'Hide' : 'Show'}
                                                >
                                                    {i.isVisible ? (
                                                        <EyeIcon className="w-4 h-4 text-gray-400 hover:text-white" />
                                                    ) : (
                                                ...
                                                <button
                                                    onClick={() => onDeleteIndicator(i.id)}
                                                    title="Delete"
                                                >
                                                    <TrashIcon className="w-4 h-4 text-gray-400 hover:text-red-400" />
                                                </button>
...
const OrderInput: React.FC<{
    ...
    return (
        <div className="flex items-center">
            <label htmlFor={inputId} className="w-16 text-gray-400 text-xs">
                {label}
            </label>
        ...
        <div className="p-4 space-y-4 text-sm">
            <div className="grid grid-cols-3 gap-1 bg-gray-900 p-1 rounded-md text-xs mb-2">
                {['Market', 'Limit', 'Stop'].map((tab) => (
                    <button
                        ...
                        className={`py-1.5 rounded ${activeTab === tab ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700/50'}`}
                    >
                        {tab}
                    </button>
                ))}
            ...
            <div className="flex items-center justify-center gap-3 py-2">
                ...
                <div className="flex flex-col items-center">
                    <input
                        ...
                        className="w-24 text-center bg-gray-800 border border-gray-700 rounded-md p-1.5 text-base font-semibold focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
            ...
            {activeTab !== 'Market' && (
                <OrderInput
                    label="Price"
                    value={order.price}
                    onChange={(val) => setOrder({ ...order, price: val })}
                />
            )}
            ...
            <div className="space-y-3 pt-2 border-t border-gray-800">
                <OrderInput
                    label="Take Profit"
                    value={order.tp}
                    onChange={(val) => setOrder({ ...order, tp: val })}
                    onPlaceLine={() => onPlaceLine('tp')}
                    isPlacing={placingOrderLine === 'tp'}
                />
                <OrderInput
                    label="Stop Loss"
                    value={order.sl}
                    onChange={(val) => setOrder({ ...order, sl: val })}
                    onPlaceLine={() => onPlaceLine('sl')}
                    isPlacing={placingOrderLine === 'sl'}
                />

                <div className="flex items-center pt-2">
                    <label htmlFor="forex-risk-percent" className="w-16 text-gray-400 text-xs">
                        Risk %
                    </label>
                    <input
                        id="forex-risk-percent"
                        type="number"
                        step="0.1"
                        value={order.riskPercent}
                        onChange={(e) => setOrder({ ...order, riskPercent: e.target.value })}
                        className="flex-1 bg-gray-800 border border-gray-700 rounded-md p-2 text-sm text-right focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                    <button
                        onClick={calculateLotsFromRisk}
                        className="ml-2 text-xs bg-gray-700 hover:bg-gray-600 px-3 py-2 rounded-md transition-colors"
                    >
                        Calc
                    </button>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-4">
                <button
                    ...
                    className="w-full bg-red-500/90 hover:bg-red-500 rounded-lg p-3 text-center transition-colors"
                >
                ...
                <button
                    ...
                    className="w-full bg-green-500/90 hover:bg-green-500 rounded-lg p-3 text-center transition-colors"
                >
                    <div className="font-bold text-white">Buy</div>
                    <div className="text-xs font-mono text-green-100">
...
const LeverageSelector: React.FC<{
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            >
                <span
                    className={`font-semibold ${marginMode === 'Cross' ? 'text-yellow-500' : 'text-purple-400'}`}
                >
                    {marginMode}
                </span>
...
const CryptoOrderPanel: React.FC<{
    ...
    const cost = useMemo(() => {
        ...
        const price =
            activeTab === 'Market' ? currentPrice : parseFloat(order.price) || currentPrice;
            ...
            <div className="p-4 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                ...
                <div className="flex justify-between items-center text-xs">
                    ...
                    <div className="flex items-center gap-1 text-gray-300">
                        <span>
                            {accountBalance.toLocaleString()} {quoteAsset}
                        </span>
                        <button
                            className="text-blue-500 hover:text-blue-400 bg-blue-500/10 rounded-full p-0.5"
                        ...
                        {showTpSl && (
                            <div className="space-y-2 pl-2 border-l-2 border-gray-800 ml-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <button
                                        onClick={() => onPlaceLine('tp')}
                                        aria-label="Place take profit on chart"
                                        className={`p-1 rounded hover:bg-gray-700 ${placingOrderLine === 'tp' ? 'text-blue-500' : 'text-gray-500'}`}
                                    >
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <button
                                        onClick={() => onPlaceLine('sl')}
                                        aria-label="Place stop loss on chart"
                                        className={`p-1 rounded hover:bg-gray-700 ${placingOrderLine === 'sl' ? 'text-blue-500' : 'text-gray-500'}`}
                                    >
                                        <TargetIcon className="w-3 h-3" />
                                    </button>
                                </div>
                        ...
                        <div className="flex items-center gap-4 pt-1">
                            <label className="flex items-center gap-1.5 cursor-pointer group">
                                <input
                                    ...
                                    onChange={(e) =>
                                        setOrder({ ...order, reduceOnly: e.target.checked })
                                    }
                                    className="rounded bg-gray-700 border-gray-600 text-blue-500 focus:ring-0 w-3 h-3"
                                />
                            ...
                            <label className="flex items-center gap-1.5 cursor-pointer group">
                                <input
                                    ...
                                    onChange={(e) =>
                                        setOrder({ ...order, postOnly: e.target.checked })
                                    }
                                    className="rounded bg-gray-700 border-gray-600 text-blue-500 focus:ring-0 w-3 h-3"
                                />
                                <span className="text-[10px] text-gray-500 group-hover:text-gray-300">
...
export const SidePanels: React.FC<SidePanelsProps> = (props) => {
    ...
    const renderPanel = () => {
        switch (props.panel) {
            ...
            case 'alerts':
                return <AlertsPanel onClose={props.onClose} />;
            case 'dataWindow':
                return (
                    <DataWindowPanel
                        onClose={props.onClose}
                        hoveredCandle={props.hoveredCandle}
                        symbol={props.symbol}
                    />
                );
            case 'objectTree':
                return (
                    <ObjectTreePanel
                        ...
                        onToggleDrawingVisibility={props.onToggleDrawingVisibility}
                        onDeleteIndicator={props.onDeleteIndicator}
                        onToggleIndicatorVisibility={props.onToggleIndicatorVisibility}
                    />
                );
            case 'orderPanel':
                return (
                    <OrderPanel
                        ...
                        placingOrderLine={props.placingOrderLine}
                        onPlaceLine={props.onPlaceLine}
                        onExecuteOrder={props.onExecuteOrder}
                        assetType={props.assetType}
                        forexAccountBalance={props.forexAccountBalance}
                        cryptoAccountBalance={props.cryptoAccountBalance}
                    />
                );
