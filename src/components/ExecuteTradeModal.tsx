...
const ExecuteTradeModal: React.FC<ExecuteTradeModalProps> = ({ signal, onClose, onExecute }) => {
    const [account, setAccount] = useState<'Forex' | 'Binance'>(
        isCryptoPair(signal.pair) ? 'Binance' : 'Forex'
    );
                ...
                <div className="flex justify-between items-center p-4 border-b border-gray-800 bg-gray-800/50 rounded-t-xl">
                    <div>
                        <h2 className="font-bold text-white text-lg flex items-center gap-2">
                            {signal.pair}
                            <span
                                className={`px-2 py-0.5 text-xs rounded-md ${signal.direction === 'BUY' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}
                            >
                ...
                <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex justify-between items-center text-center bg-gray-800/40 p-3 rounded-lg border border-gray-700/50 shadow-inner">
                            <div className="flex flex-col">
                                ...
                                <span
                                    className={`font-bold text-lg ${signal.direction === 'BUY' ? 'text-green-500' : 'text-red-500'}`}
                                >
                                    {signal.direction}
                                </span>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <div className="flex justify-between mb-1">
                                    ...
                                    <span
                                        className={`text-[10px] font-mono ${stopLoss < entryPrice ? 'text-red-400' : 'text-red-400'}`}
                                    >
                            ...
                            <div>
                                <div className="flex justify-between mb-1">
                                    ...
                                    <span
                                        className={`text-[10px] font-mono ${takeProfit > entryPrice ? 'text-green-400' : 'text-green-400'}`}
                                    >
                                        {(((takeProfit - entryPrice) / entryPrice) * 100).toFixed(
                                            2
                                        )}
                                        %
                                    </span>
                        ...
                        <div className="bg-gray-800/50 p-4 rounded-lg space-y-2 text-xs border border-gray-700/30">
                            <div className="flex justify-between">
                                ...
                                <span className="text-white font-mono">
                                    {positionSizeCoins.toFixed(4)} {signal.pair.replace('USDT', '')}
                                </span>
                            </div>
                            {isCrypto && (
                                <div className="flex justify-between">
                    ...
                    <div className="space-y-5 flex flex-col justify-between">
                        ...
                        <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-800">
                            <button
                                onClick={onClose}
                                className="py-3 rounded-lg text-sm font-semibold text-gray-400 bg-gray-800 hover:bg-gray-700 transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handleSubmit}
                                className={`py-3 rounded-lg text-sm font-bold text-white shadow-lg transition-all ${
