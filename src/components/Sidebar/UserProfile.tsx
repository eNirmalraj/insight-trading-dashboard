...
export const UserProfile: React.FC<UserProfileProps> = ({ isExpanded, onLogout }) => {
    ...
    return (
        <div className="relative border-t border-gray-700 mt-2 pt-2" ref={menuRef}>
            ...
            {isOpen && (
                <div
                    className={`absolute z-50 bg-gray-800 border border-gray-700 rounded-lg shadow-xl w-56 overflow-hidden py-1 ${isExpanded ? 'bottom-full left-0 mb-2' : 'left-full bottom-0 ml-2'}`}
                >
                    {isPro ? (
