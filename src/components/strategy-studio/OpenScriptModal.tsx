...
export const OpenScriptModal: React.FC<OpenScriptModalProps> = ({
    ...
    const [view, setView] = useState<'BUILT_IN' | 'MY_SCRIPTS'>('MY_SCRIPTS');

    const strategyScripts = savedStrategies;
    const isIndicatorMode = mode === 'indicator';
                    ...
                    {view === 'BUILT_IN' && (
                        <div>
                            <div className="text-xs font-bold tracking-wider text-gray-500 uppercase mb-3">
                                {isIndicatorMode ? 'Built-in Indicators' : 'Built-in Strategies'}
                            </div>
                            <div className="space-y-1">
                                {isIndicatorMode ? (
                                    /* Built-in Indicators from .kuri files */
                                    DEFAULT_INDICATORS.map((ind) => (
                                        <button
                                            key={ind.id}
                                            onClick={() => {
                                                onLoadHelper(ind.kuriSource || '', ind.name, ind.id);
                                                onClose();
                                            }}
                                    ...
                                    /* Built-in Strategies from Strategy Registry */
                                    STRATEGY_REGISTRY.length > 0 ? (
                                        STRATEGY_REGISTRY.map((strat) => (
                                            <button
                                                key={strat.id}
                                                onClick={() => {
                                                    onLoadHelper(strat.kuriSource, strat.name, strat.id);
                                                    onClose();
                                                }}
                                                className="w-full text-left px-4 py-3 rounded-md bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 transition-all flex items-center justify-between group"
                                            >
                                                <span className="text-gray-200 text-sm font-medium">{strat.name}</span>
                                                <span className="text-xs text-purple-400 opacity-0 group-hover:opacity-100 transition-opacity">Open</span>
                                            </button>
                                        ))
                                    ) : (
                                        <div className="text-center py-6 text-gray-500 italic text-sm">No built-in strategies.</div>
                                    )
                                )}
                            </div>
                        </div>
                    ...
                    {view === 'MY_SCRIPTS' && (
                        <div>
                            {loading ? (
                                ...
                                <>
                                    {/* User Strategies */}
                                    {strategyScripts.filter(
                                        (s) => !STRATEGY_REGISTRY.some((r) => r.name === s.name)
                                    ).length > 0 && (
                                        <div className="mb-6">
                                            <div className="text-xs font-bold tracking-wider text-gray-500 uppercase mb-3">
                                                Strategies
                                            ...
                                            <div className="space-y-1">
                                                {strategyScripts
                                                    .filter(
                                                        (s) =>
                                                            !STRATEGY_REGISTRY.some((r) => r.name === s.name)
                                                    )
                                                    .map((s) => (
                                                        <div
                                                            key={s.id}
