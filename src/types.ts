...
export interface Signal {
    id: string;                  // signal_executions.id
    pair: string;
    strategy: string;
    ...
    lotSize?: number;
    leverage?: number;
    // Scanner-side metadata (from the joined signals row)
    paramsSnapshot?: Record<string, any>;
    templateVersion?: string;
    signalEventId?: string;      // signals.id — the underlying event row
    market?: 'spot' | 'futures';
}
...
export interface Strategy {
    ...
    scriptSource?: string; // Kuri source as saved by StrategyStudio
}
