...
// Error callback for surfacing signal engine errors to UI
...
export const getFavoriteTimeframes = (): string[] => {
    ...
    console.log(
        `[SignalEngine] Using default timeframes: ${DEFAULT_MONITOR_CONFIG.timeframes.join(', ')}`
    );
...
export const runSignalGeneration = async (): Promise<EngineRunStats> => {
    ...
    try {
        ...
        try {
            ...
            console.log(
                '[SignalEngine] Top 5 Scan Targets:',
                Array.from(uniqueSymbols).slice(0, 5)
            );

            if (uniqueSymbols.size === 0) {
                // Fallback to major pairs if fetch fails or returns empty
                ['BTC/USDT.P', 'ETH/USDT.P', 'BNB/USDT.P', 'SOL/USDT.P', 'XRP/USDT.P'].forEach(
                    (s) => uniqueSymbols.add(s)
                );
            }
        } catch (e) {
        ...
        const riskSettingsMap: Record<string, RiskSettings> = {};
        try {
        ...
        console.log(
            `[SignalEngine] Scanning ${symbols.length} symbols on FAVORITE timeframes: ${Array.from(activeTimeframes).join(', ')}...`
        );
            ...
            for (const timeframe of Array.from(activeTimeframes)) {
                try {
                    ...
                    if (result.signalsCreated > 0) {
                        console.log(
                            `[SignalEngine] Generated ${result.signalsCreated} signals for ${symbol} ${timeframe}`
                        );
                    }
                } catch (error: any) {
        ...
        stats.executionTimeMs = Date.now() - startTime;
        console.log(
            `[SignalEngine] Generation complete. Signals created: ${stats.signalsGenerated}, Time: ${stats.executionTimeMs}ms`
        );
    } catch (error: any) {
...
export const updateSignalStatuses = async (): Promise<void> => {
    try {
        ...
        if (pendingSignals.length > 0) {
            console.log(
                `[SignalEngine] Checking ${pendingSignals.length} pending signals for entry triggers...`
            );
            ...
            for (const signal of pendingSignals) {
                try {
                    ...
                    if (signal.entryType === EntryType.LIMIT) {
                        ...
                        entryTriggered = true;
                        console.log(
                            `[SignalEngine] ⚡ Activating pending Market signal: ${signal.pair}`
                        );
                    }
                    ...
                    if (entryTriggered) {
                        console.log(
                            `[SignalEngine] 🎯 Entry triggered: ${signal.pair} ${signal.direction} @ ${currentPrice.toFixed(2)} (Entry: ${signal.entry.toFixed(2)})`
                        );
                        await updateSignalStatus(signal.id, SignalStatus.ACTIVE);
                    }
                } catch (error: any) {
                    console.error(
                        `[SignalEngine] Error checking pending signal ${signal.id}:`,
                        error.message
                    );
                }
            }
        ...
        console.log(
            `[SignalEngine] Monitoring ${activeSignals.length} active signals for TP/SL...`
        );
        ...
        for (const signal of activeSignals) {
            try {
                ...
                if (tpHit) {
                    console.log(
                        `[SignalEngine] ✅ Take Profit hit for ${signal.pair} signal ${signal.id}`
                    );
                    ...
                    console.log(
                        `[SignalEngine] ❌ Stop Loss hit for ${signal.pair} signal ${signal.id}`
                    );
                    const { closeSignal } = await import('../services/signalService');
                    ...
                    if (newSL !== signal.stopLoss) {
                        console.log(
                            `[SignalEngine] 🛡️ Trailing SL adjusted for ${signal.pair} from ${signal.stopLoss.toFixed(2)} to ${newSL.toFixed(2)}`
                        );
                        const { updateSignalRiskLevels } =
                            await import('../services/signalService');
