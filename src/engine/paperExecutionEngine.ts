...
export class PaperExecutionEngine {
    ...
    public static async processSignal(signal: Signal) {
        ...
        if (!eligibilities || eligibilities.length === 0) {
            console.log(
                `[PaperExecution] Signal ${signal.id} (${signal.pair}) skipped. No eligible auto-trade watchlists found.`
            );
            return;
        }
        ...
        for (const eligibility of eligibilities) {
            console.log(
                `[PaperExecution] 🚀 Opening position for user ${eligibility.userId} on watchlist ${eligibility.watchlistName}`
            );
            await this.openPosition(signal, eligibility.riskSettings, eligibility.userId);
        }
    }

    private static async checkAutoTradeEligibility(
        signal: Signal
    ): Promise<{ userId: string; watchlistName: string; riskSettings: any }[] | null> {
        ...
        console.log(
            `[PaperExecution] Searching for watchlists matching strategy: ${targetStrategyName}...`
        );

        // 2. Find ALL scripts (watchlists) that use this strategy
        const { data: watchlists, error: wlError } = await db()
            .from('watchlists')
            .select(
                `
                id,
        ...
        if (!watchlists || watchlists.length === 0) {
            console.log(
                `[PaperExecution] No watchlists found matching strategy '${targetStrategyName}' and mode 'paper'`
            );
            return null;
        }

        console.log(
            `[PaperExecution] Found ${watchlists.length} potential watchlists. Checking symbol items...`
        );
        ...
        for (const wl of watchlists) {
            ...
            console.log(
                `[PaperExecution] Checking watchlist '${wl.name}'. Items: ${wl.items.length}, Enabled: ${enabledItems.length}`
            );

            // Check if symbol is enabled in this watchlist
            const isItemEnabled = wl.items.some(
                (item: any) => item.symbol === signal.pair && item.auto_trade_enabled === true
            );
            ...
            if (
                wl.execution_timeframes &&
                Array.isArray(wl.execution_timeframes) &&
                wl.execution_timeframes.length > 0
            ) {
                if (!wl.execution_timeframes.includes(signal.timeframe)) {
                    console.log(
                        `[PaperExecution] Watchlist '${wl.name}' timeframe mismatch. Skip.`
                    );
                    continue;
                }
            }
    ...
    private static async openPosition(signal: Signal, riskSettings: any, userId: string) {
        try {
            ...
            if (riskSettings?.manualRiskEnabled) {
                ...
                overrides = {
                    stopLoss:
                        distSl > 0 ? (isBuy ? entry - distSl : entry + distSl) : signal.stopLoss,
                    takeProfit:
                        distTp > 0 ? (isBuy ? entry + distTp : entry - distTp) : signal.takeProfit,
                ...
                console.log(
                    `[PaperExecution] Applying Manual Risk Override for ${signal.pair}. SL: ${overrides.stopLoss}`
                );
            }
            ...
            const isCrypto =
                riskSettings?.accountType?.toLowerCase() === 'crypto' || riskSettings?.marketType;
            ...
            if (isCrypto) {
                    ...
                    if (isFutures) {
                        const direction =
                            ...
                            signal.direction.toLowerCase() === 'long'
                                ? 'buy'
                                : 'sell';
                        const liqSafety = RiskCalculator.checkLiquidationSafety(
                            entry,
                            overrides.stopLoss,
                            finalLeverage,
                            direction
                        ...
                        if (!liqSafety.isSafe) {
                            console.error(
                                `[PaperExecution] Trade Rejected: ${liqSafety.reason} (Liq: ${liqSafety.liquidationPrice.toFixed(2)})`
                            );
                            return;
                        }
                    }
                ...
                if (riskMethod === 'fixed') {
                    ...
                    try {
                        ...
                        if (!cryptoAccount) {
                            console.error(
                                `[PaperExecution] No Crypto/Binance account found for user ${userId}. Skipping.`
                            );
                            return;
                        }
                        ...
                        if (balance <= 0) {
                            console.warn(
                                `[PaperExecution] Insufficient balance (${balance}). Skipping.`
                            );
                            return;
                        }
                        ...
                        const { data: openPositions, error: posError } = await db()
                            .from('paper_trades')
                        ...
                        if (!posError && openPositions) {
                            ...
                            if (totalOpenRisk + newRisk > maxRiskAmt) {
                                console.warn(
                                    `[PaperExecution] Portfolio Risk Cap Exceeded! Open: $${totalOpenRisk.toFixed(2)}, New: $${newRisk.toFixed(2)}, Limit: $${maxRiskAmt.toFixed(2)}. Skipping.`
                                );
                                return;
                            }
                        }
                        ...
                        if (riskPerc > 100 || riskPerc <= 0) {
                            console.warn(
                                `[PaperExecution] Invalid Risk Percent (${riskPerc}). Skipping.`
                            );
                            return;
                        }
                        ...
                        if (slDistance > 0) {
                            ...
                            console.error(
                                `[PaperExecution] CRITICAL: SL Distance is 0. Cannot calculate quantity from Risk %. Skipping.`
                            );
                        ...
                        if (isFutures && riskSettings?.autoLeverageEnabled) {
                            ...
                            console.log(
                                `[PaperExecution] Auto-Leverage calculated: ${finalLeverage}x (Pos: ${positionValue.toFixed(2)}, Bal: ${balance})`
                            );
                        }
                        ...
                        if (slDistance > 0 && overrides?.stopLoss) {
                            ...
                            quantity = RiskCalculator.calculateFeeAwareQty(
                                riskAmount,
                                entry,
                                overrides.stopLoss
                            );
                            console.log(
                                `[PaperExecution] Fee-Aware Sizing applied. Qty adjusted for fees.`
                            );
                        }
                    } catch (err) {
            ...
            if (overrides) {
                if (overrides.stopLoss)
                    overrides.stopLoss = SymbolUtils.applyTickSize(
                        overrides.stopLoss,
                        precision.tickSize
                    );
                if (overrides.takeProfit)
                    overrides.takeProfit = SymbolUtils.applyTickSize(
                        overrides.takeProfit,
                        precision.tickSize
                    );
                if (overrides.trailingStopLoss)
                    overrides.trailingStopLoss = SymbolUtils.applyTickSize(
                        overrides.trailingStopLoss,
                        precision.tickSize
                    );
            }
            ...
            if (quantity < precision.minQty) {
                console.warn(
                    `[PaperExecution] Quantity ${quantity} below MinQty ${precision.minQty} for ${signal.pair}. Skipping.`
                );
                return;
            }
            ...
            if (quantity <= 0 || !isFinite(quantity)) {
                console.error(
                    `[PaperExecution] Invalid Quantity Calculated: ${quantity}. Skipping.`
                );
                return;
            }

            console.log(
                `[PaperExecution] Executing Trade: ${signal.pair} | Qty: ${quantity} | Lev: ${finalLeverage}x | SL: ${overrides?.stopLoss}`
            );
