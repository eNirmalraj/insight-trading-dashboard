...
export class ChartEngine {
    ...
    public setSelectedDrawing(drawingId: string | null) {
        this.selectedDrawingId = drawingId;
        this.render();
    }

    public getSelectedDrawing(): string | null {
        return this.selectedDrawingId;
    }

    public setHoveredDrawing(drawingId: string | null) {
        if (this.hoveredDrawingId !== drawingId) {
            this.hoveredDrawingId = drawingId;
            this.render();
        }
    }
    ...
    public render() {
        ...
        */
        ...
        // Draw drawings with selection/hover state
        this.drawingRenderer.drawDrawings(
            this.drawings,
        ...
        if (this._crosshairPixels) {
            this.crosshairRenderer.draw(
                this._crosshairPixels.x,
                this._crosshairPixels.y,
                { width: this.width, height: this.height },
                {
                    color: '#555',
                    labelColor: '#fff',
    ...
    private drawSeries() {
        this.series.forEach((s) => {
            ...
            // Since the Bridge creates SeriesData from Kuri output which is 1:1 with input candles,
            ...
            if (s.type === 'line') {
                this.lineRenderer.draw(
                    values,
                    this.view,
                    this.coordinateSystem as CoordinateSystem,
                    s.color,
                    s.lineWidth || 2
                );
