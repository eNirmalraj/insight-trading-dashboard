...
export class ScriptMarketplace {
     ...
     */
    async addComment(userId: string, scriptId: string, content: string, parentId?: string): Promise<void> {
        const { error } = await supabaseAdmin
    ...
    async forkScript(originalScriptId: string, userId: string): Promise<string> {
        ...
        if (relationError) console.error(`Failed to record fork relationship: ${relationError.message}`); // Non-critical
     ...
     */
    async reportScript(userId: string, scriptId: string, reason: string, details: string): Promise<void> {
        const { error } = await supabaseAdmin
            .from('script_reports')
            .insert({
                reporter_id: userId,
     ...
     */
    async suggestImprovement(userId: string, scriptId: string, title: string, description: string): Promise<void> {
        const { error } = await supabaseAdmin
