...
(function (root) {
        ...
        parseStatement() {
            ...
            if (t.type === T.KEYWORD) {
                switch (t.value) {
                    ...
                    case 'plot':
                        throw new Error(
                            `Parse Error L${t.line}: plot() is deprecated. Use mark() instead. Example: mark(val, title="Name", color=#2962FF)`
                        );
                    case 'plotshape':
                        throw new Error(
                            `Parse Error L${t.line}: plotshape() is deprecated. Use mark.shape() instead.`
                        );
                    case 'plotchar':
                        throw new Error(
                            `Parse Error L${t.line}: plotchar() is deprecated. Use mark.shape() instead.`
                        );
                    case 'plotarrow':
                        throw new Error(
                            `Parse Error L${t.line}: plotarrow() is deprecated. Use mark.arrow() instead.`
                        );
                    case 'hline':
                        throw new Error(
                            `Parse Error L${t.line}: hline() is deprecated. Use mark.level() instead.`
                        );
                    case 'bgcolor':
                        throw new Error(
                            `Parse Error L${t.line}: bgcolor() is deprecated. Use mark.bgcolor() instead.`
                        );
                    case 'fill':
                        throw new Error(
                            `Parse Error L${t.line}: fill() is deprecated. Use mark.fill() instead.`
                        );
                    case 'draw':
                        throw new Error(
    ...
    const drawingFunctions = {
        'line.new': (a) => {
            ...
            return new DrawingLine(a[0], a[1], a[2], a[3], opts);
        },
        'line.delete': (a) => {
        ...
        'label.new': (a) => {
            ...
            return new DrawingLabel(a[0], a[1], a[2] || '', opts);
        },
        'label.delete': (a) => {
            if (a[0] && a[0].delete) a[0].delete();
        },
        ...
        'box.new': (a) => {
            ...
            return new DrawingBox(a[0], a[1], a[2], a[3], opts);
        },
        'box.delete': (a) => {
            if (a[0] && a[0].delete) a[0].delete();
        },
    ...
    const arrayFunctions = {
        ...
        'array.every': (a) => false, //simplified
        'array.some': (a) => false,
    ...
    class KuriInterpreter {
        ...
        eval(node, env, bar) {
            ...
            switch (node.type) {
                ...
                case N.HistoryExpression: {
                    ...
                    const sn = node.source.type === N.Identifier ? node.source.name : null;
                    if (sn && this.seriesData.has(sn)) return this.seriesData.get(sn)[tb];
                    return NaN;
                }
        ...
        evalRequestSecurity(node, args, env, bar) {
            ...
            // Compute target timeframe in milliseconds
            ...
            // Group bars into HTF windows
            const htfBars = []; // { o, h, l, c, v, t, startIdx, endIdx }
            let cur = null;
            for (let i = 0; i < times.length; i++) {
                const windowStart = Math.floor(times[i] / tfMs) * tfMs;
